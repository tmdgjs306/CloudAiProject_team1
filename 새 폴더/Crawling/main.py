...
class AutoCrawler:
    def __init__(self, skip_already_exist=True, n_threads=4, do_google=True, do_naver=True, download_path='download',
        ...
        os.makedirs('./{}'.format(self.download_path), exist_ok=True)  # 다운로드 경로 생성
    ...
    # 키워드 파일에서 키워드 읽기
    @staticmethod
    def get_keywords(keywords_file='Search.txt'):
        # 파일에서 검색 키워드 읽기
    ...
    def download_from_site(self, keyword, site_code):
        ...
        try:
            ...
            collect = CollectLinks(no_gui=self.no_gui, proxy=proxy)  # Chrome 드라이버 초기화
        except Exception as e:
